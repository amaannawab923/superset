...
const memoizedGetColorFormatters = memoize(
  (
    columnConfig: ConditionalFormattingConfig[] | undefined,
    data: DataRecord[],
    alpha: boolean | undefined,
    _,
  ) => getColorFormatters(columnConfig, data, alpha),
  (
    columnConfig: ConditionalFormattingConfig[] | undefined,
    _,
    alpha: boolean | undefined,
    queryCacheKey: string | null,
  ) => queryCacheKey + JSON.stringify([columnConfig, alpha]),
);
...
const processComparisonTotals = memoizeOne(
  (comparisonSuffix: string, totals?: DataRecord[]): DataRecord | undefined => {
    if (!totals) {
      return totals;
    }
...
const processComparisonDataRecords = memoize(
  function processComparisonDataRecords(
    originalData: DataRecord[] | undefined,
    originalColumns: TableDataColumnMeta[],
  ...
  (_, originalColumns, comparisonSuffix, queryCacheKey) =>
    queryCacheKey + JSON.stringify([originalColumns, comparisonSuffix]),
);
...
const processColumns = memoizeOne(function processColumns(
  ...
  const columns: TableDataColumnMeta[] = (colnames || [])
    .filter(
      key =>
...
const transformProps = (
  ...
  const {
    ...
    show_raw_record_totals: showRawRecordTotals,
  ...
  let comparisonColumns: TableDataColumnMeta[] = [];
  ...
  const [baseQuery, totalQuery] = queriesData;
  const rowCount = baseQuery?.rowcount ?? 0;

  const queryCacheKey = baseQuery?.cache_key ?? JSON.stringify(baseQuery?.data);
  ...
  if (Array.isArray(table_state?.columnOrder?.orderedColIds)) {
    let lastMatchingIndex = -1;
    passedColumns.forEach(col => {
      if (!table_state.columnOrder.orderedColIds.includes(col.key)) {
        table_state.columnOrder.orderedColIds.splice(
          lastMatchingIndex + 1,
          0,
          col.key,
        );
        ...
        lastMatchingIndex = table_state.columnOrder.orderedColIds.indexOf(
          col.key,
        );
      }
    });
